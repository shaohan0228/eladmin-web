    ...
    <el-main class="workorder-main">
      <div class="main-con">
        ...
        <div class="padd20">
          ...
          <div class="g666 fsize12 mtop15" v-html="detailsData.create_time">2021-01-01 13:00:00</div>
          <div class="problem-tit mtop40">问题描述</div>
          <div class="problem-ms mtop15" v-html="orderDesc" />
          <div class="problem-tit mtop40">联系方式</div>
          <div class="problem-infor mtop15">
            <span>联系人：{{ detailsData.customer_info }}</span>
            <span>联系方式：{{ detailsData.customer_phone }}</span>
          ...
          <div class="mtop20">
            ...
            <span v-html="detailsData.describe">
          ...
          <div class="mtop20">
            <el-timeline class="mtop20">
              <el-timeline-item v-for="(item, index) in detailsData.log" :key="index" placement="top">
                <el-card>
                  <h4>
                    <span v-html="item.opinion">如何解决工单异常问题</span>
                    <span v-if="item.status === '1'" class="gdtimelinezt ba2F489C">已接单</span>
                    <span v-else-if="item.status === '2'" class="gdtimelinezt baFF8E3D">处理中</span>
                    <span v-else-if="item.status === '3'" class="gdtimelinezt ba67c23a">已完成</span>
                    <span v-else class="gdtimelinezt ca1824">待接单</span>
                  </h4>
                  <p class="fsize12 g999">处理人：
                    <span v-html="item.operate_user">刘某</span>
                  </p>
                  <p class="fsize12 g999" v-html="item.operate_dt">2020-12-06 13:00:02</p>
                </el-card>
              </el-timeline-item>
            </el-timeline>
          </div>
        ...
        <el-dialog title="结单" :visible.sync="dialogFormVisible">
          <el-form :model="form">
            <el-form-item label="处理意见">
              <el-input v-model="form.message" type="textarea" placeholder="请输入您的处理意见" class="w70" />
            </el-form-item>
          </el-form>
...
<script>
import { getWorkOrderDetails } from '@/api/work/work'
...
export default {
  ...
  methods: {
    async getWorkOrderDetails() {
      const result = await getWorkOrderDetails(this.id)
      console.log(result.data.mes)
      if (result.code === 200) {
        this.detailsData = result.data.mes
      }
    },
    receivingOrder() {
